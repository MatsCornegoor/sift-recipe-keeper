...
async function getCachedExtraction(modelId, url, cleanContent, cache) {
  const key = cacheKey(modelId, url);
  if (cache[key]) {
    process.stdout.write('(cached) ');
    return cache[key].recipe;
  }

  const recipe = await extractRecipe(modelId, cleanContent, API_KEY);
  cache[key] = { extractedAt: new Date().toISOString(), recipe };
  saveCache(cache);
  return recipe;
}
...
async function main() {
  ...
  console.log(`Extraction cache: ${cacheHits} entr${cacheHits === 1 ? 'y' : 'ies'}`);
    ...
    for (const group of section.groups) {
      ...
      for (const modelId of group.models) {
        ...
        for (let i = 0; i < TEST_URLS.length; i++) {
          ...
          try {
            const extracted = await getCachedExtraction(modelId, url, cleanContent, cache);
            process.stdout.write('rating... ');
            const { mistakes, score } = await rateExtraction(extracted, groundTruth);
